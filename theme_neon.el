;;; neon --- theme
;;; Commentary:
;;; Code:

(load-file "~/.emacs.d/theme_variables.el")

;#bf4092

(setq background "#000000")
(setq background-fringe "#331933")
(setq background-hl "#331933")
(setq background-magit "#331933")
(setq background-magit-dark "#000000")
(setq background-mode-line "#331933")
(setq background-mode-line-active "#561056")
(setq background-low "#440044")
(setq background-region "#a68124")
(setq text-constant "#545299")
(setq text-function "#339933")
(setq text-highlight-1 "#1eb68a")
(setq text-highlight-2 "#00dd00")
(setq text-info "#555577")
(setq text-low "#8f3c7f")
(setq text-lower "#684962")
(setq text-mode-line "#ccaaaa")
(setq text-mode-line-buffer "#cdabab")
(setq text-normal "#bb7777");sdfsdfds
(setq text-preprocessor "#00ff00")
(setq text-type "#bb4f80")
(setq text-variable "#e49e59")
(setq text-whitespace "#8f3c7f")

;;; theme_neon.el ends here
