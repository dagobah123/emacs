...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my-git-grep-search (search-strings)
    "Search for multiple strings using vc-git-grep and display simplified output."
    (interactive "MEnter search strings (comma-separated): ")
    (let* ((search-list (split-string search-strings "," t " "))
           (default-directory (vc-git-root default-directory))
           (grep-command (format "git --no-pager grep -n -E -i -e %s -- %s"
                                 (mapconcat 'shell-quote-argument search-list ".*")
      ...
      (with-current-buffer grep-buffer-name
        (while (re-search-forward (concat "^" (mapconcat 'identity search-list ".*")) nil t)
          (replace-match "")))
      (goto-char (point-min))
      (delete-non-matching-lines (rx bol (group (1+ digit)) ":" (group (1+ not-newline)) eol))))
  (global-set-key (kbd "C-c f") 'my-git-grep-search)
