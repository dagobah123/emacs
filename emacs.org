...
#+BEGIN_SRC emacs-lisp
  (defun my:find-files-in-project (directory extension)
    ...
    (let ((file-list '()))
      ...
      (if file-list
          (progn
            (message "Files found with extension %s:" extension)
            (dolist (file file-list)
              (message "%s" file)))
        (message "No files found with extension %s in %s" extension directory))
      (message "my:find-files-in-project() Length of the list: %d" (length file-list))
      file-list))

  (defun get-java-parents (file-path)
  ...
  (defun parse-java-file-for-members (file-path)
    "Parse a Java file to extract member types (fields, methods) with variable names."
    (with-temp-buffer
      (insert-file-contents file-path)
      (goto-char (point-min))
      (let (member-data)
        (while (re-search-forward "\\b\\(\\(?:public\\|private\\|protected\\)?\\)\\s-+\\(\\(?:[[:alnum:]_$]+\\.\\)*[[:alnum:]_$]+\\)\\s-*\\([[:alnum:]_$]+\\)\\s-*\\(([^)]*)\\)?\\s-*\\({\\|;\\)" nil t)
          (let ((access-modifier (match-string 1))
                (type (match-string 2))
                (name (match-string 3))
                (variable-names (and (match-beginning 4) (match-end 4) (buffer-substring (match-beginning 4) (match-end 4)))))
            (setq member-data (cons (list access-modifier type name variable-names) member-data))))
        member-data)))
      ...
      (if project-root
          (let ((file-list (my:find-files-in-project project-root "java")))
            ...
            (cl-loop for element in file-list
                     for index from 0
                     do (message "Index: %d, Element: %s" index element (get-java-parents element))

                     (dolist (element2 (get-java-parents element))
                       (message "Parent: %s" element2))
                     )
            )
