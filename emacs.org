...
#+BEGIN_SRC emacs-lisp
  ...
  ^^^^^-------------------------------------------------------------------------------------------------
  _i_: imenu        _g_: status     _1_: dired project   _p_: root   _E_: buffers      _-_: compile
  _s_: symbols      _l_: log        _2_: vc-git-grep     _f_: files  _A_: directories  _=_: lsp
  _r_: references   _L_: log file   _3_: helm-projectile ^ ^         _n_: branches     ^ ^
  _d_: definition   _b_: blame      _4_: helm-git-grep   ^ ^         _m_: magit-diff   ^ ^
  _e_: errors       _B_: region     _5_: buffers         ^ ^         ^ ^               ^ ^
  _R_: references   ^ ^             ^ ^                  ^ ^         ^ ^               ^ ^
  _I_: implemen.    ^ ^             ^ ^                  ^ ^         ^ ^               ^ ^
  _c_: call hier.   ^ ^             ^ ^                  ^ ^         ^ ^               ^ ^
  _t_: type hier.   ^ ^             ^ ^                  ^ ^         ^ ^               ^ ^
  ...
  "
    ...
    ("R" (lsp-treemacs-references (isearch-forward-symbol-at-point)))
    ("I" (lsp-treemacs-implementations (isearch-forward-symbol-at-point)))
    ("c" (lsp-treemacs-call-hierarchy (isearch-forward-symbol-at-point)))
    ("t" (lsp-treemacs-type-hierarchy (isearch-forward-symbol-at-point)))
    ...
    ("b" (magit-blame))
    ("B" (magit-file-dispatch)) ;show which commits modified a region
...
#+BEGIN_SRC emacs-lisp
  (defun sandbox ()
    "Sandbox function to test various code snippets."
    (interactive)
...
(defface my:project-face
  '((t :foreground "black"))
  "Custom face for projectile-project-name in mode-line."
  :group 'my)

(defface my:buffer-face
  '((t :foreground "black"))
  "Custom face for buffer in mode-line."
  :group 'my)

(defface my:branch-face
  '((t :foreground "black"))
  "Custom face for vc-mode in mode-line."
  :group 'my)

(defun my:project-mode-icon ()
  (let ((icon (all-the-icons-octicon "bug" :height 0.7 :v-adjust 0.05 :face 'my:project-face)))
    (if (> (length icon) 0)
        (concat icon "")
      "")))

(defun my:major-mode-icon ()
  (let ((icon (all-the-icons-icon-for-mode major-mode :height 0.7 :v-adjust -0.05 :face 'my:buffer-face)))
    (if (> (length icon) 0)
        (concat icon " ")
      "")))

(defun my:vc-mode-icon ()
  ;(let ((icon (all-the-icons-octicon "git-branch" :height 0.7 :v-adjust 0.05 :face 'my:branch-face)))
  (let ((icon (all-the-icons-alltheicon "git" :height 0.7 :v-adjust 0.05 :face 'my:branch-face)))
    (if (> (length icon) 0)
        (concat icon "")
      "")))

;(defun my:vc-mode-string ()
;  (let ((icon (all-the-icons-octicon "git-branch" :face 'my:branch-face)))
;    (concat icon (format " %s" '(vc-mode vc-mode)))))

;(setq-default mode-line-format
;              (list
;               ;; Other mode-line elements here
;               '(:eval (if vc-mode
;                           (propertize (substring vc-mode (+ 2 (length " Git")))
;                                       'face 'custom-vc-mode-face)
;                         ""))))
...
(setq-default mode-line-format
              '("%e"
                ...
                (:eval (format "%s" (propertize (projectile-project-name) 'face 'my:project-face)))
                " "
                (:eval (list (my:major-mode-icon)
                        mode-line-buffer-identification))
                (:eval (if vc-mode (list (my:vc-mode-icon)
                                         (propertize vc-mode 'face 'my:branch-face))))
                " "
                mode-line-position
                ;mode-line-modes
                mode-line-misc-info
...
; Revert file when branch was changed
