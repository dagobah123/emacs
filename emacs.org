...
(defvar my:custom-mode-line-string "hello"
  "A custom string to be displayed in the mode-line.")

(defun my:check-if-empty (str)
  "Check if STR is an empty string and print a message."
...
(defun my:mode-line-format ()
  ...
  (setq-default mode-line-format
                (list
                 ...
                 '(:eval (propertize my:custom-mode-line-string 'face 'font-lock-regexp-grouping-backslash)) ;; Jave method name
                 (propertize " %b " 'face 'font-lock-regexp-face) ;; buffer
                 ;;'(:eval (propertize (substring vc-mode 5) 'face 'font-lock-constant-face)) ;; git branch
                 '(:eval (propertize (my:check-if-empty (substring vc-mode 5)) 'face 'font-lock-regexp-grouping-construct)) ;; git branch
                 (propertize " (%p,%l,%c) " 'face 'font-lock-constant-face) ;; position
                 '(:eval (propertize (format-time-string " %b %d %H:%M ") 'face 'font-lock-constant-face))
                 (propertize " %m " 'face 'font-lock-constant-face) ;; major mode
                 "  "
...
#+END_SRC
** Show current method in mode-line
#+BEGIN_SRC emacs-lisp
;(defun my:next-line ()
;  "Custom next line"
;  (interactive)
;  (next-line)
;  (my:display-defun-name))
; 
...
;  (my:display-defun-name))
; 
;(defun my:display-defun-name ()
;  "Custom display defun name."
;  (interactive)
...
;;(defun my:c-display-defun-name (&optional arg)
;;  "Display the name of the current CC mode defun.
;;   With a prefix arg, push the name onto the kill ring too."
;;  (interactive "P")
;;  (if (eq major-mode 'java-mode)
;;      (c-with-string-fences
...
;;(add-hook 'post-command-hook 'my:mode-line-format)
(add-hook 'post-command-hook 'my:c-display-defun-name)

;;(defun my:mode-line-format ()
;;  "Custom mode-line format to include the current function name and buffer status."
;;  (setq mode-line-format
;;        (list
;;         '(:eval (format "%s" (if (buffer-modified-p) "*" "-")))  ;; Shows `*` if the buffer is modified, `-` otherwise.
;;         '(:eval (format " %s" my:c-display-defun-name))
;;         "  %b  %p  %l:%c  %m")))

;;(defun my:mode-line-format ()
;;  "Custom mode-line format to include the current function name."
;;  (setq mode-line-format
;;        (list
;;         '(:eval (format " %s" (my:c-display-defun-name)))
;;         ;;'(:eval (format " %s" "hello"))
